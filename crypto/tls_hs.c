...
static int tls_msg_sh_build(struct tls_hs *tls, struct tls_vec *x, struct tls_vec *y, u8 req)
{
	...
	p = tls->omsg.data;
	p = tls_put_num(p, req ? TLS_MT_HELLO_RETRY_REQUEST : TLS_MT_SERVER_HELLO, 1);
	len_p = p;
	p += 3; /* set length later */
	p = tls_put_num(p, TLS_MSG_legacy_version, 2);
	p = tls_put_data(p, &tls->h.random);
...
static int tls_ext_psk_handle(struct tls_hs *tls, u8 mt, struct tls_vec *e)
{
	...
	if (!psk)
		return -EINVAL;
	tls_vec(&vec, tls->imsg.data, (u32)(e->data - tls->imsg.data));
...
{
	struct tls_vec x_v, y_v, s, msg = *m;
	u8 x[32], y[32], req = 0;
	u64 exts = 0;
	int err;
	u32 n;
	...
	if (tls_get_num(m, 2, &n) || tls_get_data(m, 32, &s) ||
	    tls_get_sub(m, 1, &s) || tls_get_sub(m, 2, &s))
		return -EINVAL;
	while (s.len > 0) {
		if (tls_get_num(&s, 2, &n))
			return -EINVAL;
	...
	if (tls_get_sub(m, 2, &s) || !s.len)
		return -EINVAL;
	if (tls_vec_set(&tls->buf[TLS_B_CH], msg.data - 4, msg.len + 4))
		return -ENOMEM;
...
static int tls_msg_sh_handle(struct tls_hs *tls, struct tls_vec *m)
{
	...
	if (tls_get_sub(m, 1, &s) || tls_get_sub(m, 2, &s) || !s.len)
		return -EINVAL;
	if (tls_vec_set(&tls->buf[TLS_B_SH], msg.data - 4, msg.len + 4))
		return -ENOMEM;
...
static int tls_msg_ee_handle(struct tls_hs *tls, struct tls_vec *m)
{
	...
	if (tls_get_sub(m, 2, &s) || !s.len)
		return -EINVAL;
	if (tls_vec_set(&tls->buf[TLS_B_EE], msg.data - 4, msg.len + 4))
		return -ENOMEM;
...
static int tls_msg_crt_handle(struct tls_hs *tls, struct tls_vec *m)
{
	...
	return tls_vec_set(&tls->buf[t], msg.data - 4, msg.len + 4);
}

static int tls_msg_crtvfy_handle(struct tls_hs *tls, struct tls_vec *m)
{
	struct tls_vec sig, msg = *m;
	u32 n, t;
	int err;
...
static int tls_msg_crtreq_handle(struct tls_hs *tls, struct tls_vec *m)
{
	...
	if (tls_get_sub(m, 1, &s) || tls_get_sub(m, 2, &s))
		return -EINVAL;
	tls->crt_req = 1;
	if (tls_vec_set(&tls->buf[TLS_B_CRT_REQ], msg.data - 4, msg.len + 4))
		return -ENOMEM;
...
static int tls_msg_sfin_handle(struct tls_hs *tls, struct tls_vec *m)
{
	...
	if (tls_vec_cmp(&sf_v, m))
		return -EINVAL;
	if (tls_vec_set(&tls->buf[TLS_B_S_FIN], msg.data - 4, msg.len + 4))
		return -ENOMEM;
...
static int tls_msg_cfin_handle(struct tls_hs *tls, struct tls_vec *m)
{
	...
	if (tls_vec_cmp(&cf_v, m))
		return -EINVAL;
	if (tls_vec_set(&tls->buf[TLS_B_C_FIN], msg.data - 4, msg.len + 4))
		return -ENOMEM;
	err = tls_keys_rm_setup(tls);
	if (err)
		return err;
...
static int tls_gen_ea_handle(struct tls_hs *tls, u32 type, struct tls_vec *i, struct tls_vec *o, u64 *eseq)
{
	struct tls_vec vec;
	int err;
...
static int tls_gen_ap_handle(struct tls_hs *tls, u32 type, struct tls_vec *i, struct tls_vec *o, u64 *rseq)
{
	struct tls_vec vec;
	int err;
...
struct tls_hs *tls_sk_handshake(struct socket *sock, struct tls_vec *es, char *subsys, u8 flag)
{
	...
	while (1) { /* recv hs msg and wait handshake done */
		...
		err = tls_gen_msg_handle(tls, &vec, &out, es, &er, is_serv, &rseq, &wseq, &eseq); /* handle hs msg */
		if (err < 0)
			goto err;
		if (out.len > 0) {
			memset(&msg, 0, sizeof(msg));
