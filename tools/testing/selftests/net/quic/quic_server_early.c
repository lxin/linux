...
int main(void)
{
	int sd, ret, buf_len, ad, msg_flags;
	struct sockaddr_in s_addr, c_addr;
	unsigned int addr_len, len;
	char s_msg[MSG_LEN + 1];
	...
	sleep(5);
	if (setsockopt(ad, SOL_QUIC, QUIC_SOCKOPT_NEW_TICKET, &i, sizeof(i)) < 0) {
		printf("Unable to setsockopt new ticket %d\n", errno);
		return -1;
	}
...
next:
	...
	ret = quic_sendmsg(ad, s_msg, strlen(s_msg), MSG_EOR, 1);
	if (ret == -1) {
		printf("send %d %d\n", ret, errno);
		return 1;
	}
