...
#define PSK_FILE	"cs_psks/early.psk"
int get_psk(char **buf)
{
	int fd = open(PSK_FILE, O_RDONLY);
	struct stat sb;

	if (fd == -1)
		return -1;
	if (stat(PSK_FILE, &sb) == -1)
		return -1;
...
int quic_sendto(int s, const void *msg, size_t len, struct sockaddr *addr, uint32_t addr_len,
...
int main(void)
{
	int sd, fd, ret, events, buf_len, msg_flags;
	char s_msg[2049], c_msg[2049], name[32];
	...
	memset(c_msg, 'c', sizeof(c_msg) - 1);
	ret = quic_sendto(sd, c_msg, strlen(c_msg), (struct sockaddr *)&s_addr,
			  sizeof(s_addr), 0, 0);
	if (ret == -1) {
		printf("send %d %d\n", ret, errno);
		goto out;
